...
def prompt_filter(init_prompt, scene_output_path, npy_path, mask_threshold, args):
...
def perform_3dsegmentation(xyz, keep_idx, scene_output_path, npy_path, args):
    ...
    for i, (npy_file) in enumerate(tqdm(npy_path)):
        ...
        mapping = compute_mapping(xyz, args.data_path, frame_id)
...
def get_args():
    ...
    # path arguments:
    parser.add_argument('--data_path', default="/data/luoly/dataset/assist/530_scannet_table0_copy", type=Path, help='Path to the dataset containing ScanNet 2d frames and 3d .ply files.')
    ...
    parser.add_argument('--post_floor', type=bool, default=False, help='Whether post-processing the floor')
...
if __name__ == "__main__":
    ...
    args.pred_path = args.data_path / "sampro3d_outputs" / "sampro3d_predictions"
    args.output_vis_path = args.data_path / "sampro3d_outputs" / "result_visualization"
    ...
    scene_plypath = os.path.join(args.data_path, 'dense_pc.ply')
    ...
    mask_threshold = 0.0
    keep_idx = prompt_filter(init_prompt, scene_output_path, points_npy_path, mask_threshold, args)
    ...
    pred_file = os.path.join(args.pred_path, 'seg.npy')
    ...
    output_vis_file = os.path.join(args.output_vis_path, 'seg.ply')
