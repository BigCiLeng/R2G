...
def process_imgs(cfg: DictConfig):
    # general setup
    ...
    if img_dir is None:
        ...
        exit()
    img_fold_name = path.splitext(img_dir)[0]

    print(f'Opening imgs {img_dir}')
    # cap = cv2.VideoCapture(video)
    imgs = sorted(os.listdir(img_dir), key=lambda a: int(os.path.basename(a).split('.')[0][-5:]))

    total_frame_count = len(imgs)
    ...
    with torch.inference_mode():
        with torch.amp.autocast(device, enabled=(use_amp and device == 'cuda')):
            ...
            for mask_name in all_mask_frames:
                ...
                # _, frame = cap.read()
                frame = cv2.imread(os.path.join(img_dir, imgs[frame_number]))
                if frame is None:
                    break
    ...
    with torch.inference_mode():
        with torch.amp.autocast(device, enabled=(use_amp and device == 'cuda')):
            ...
            pbar.set_description(f'Processing video {imgs}')
            for current_frame_index in range(len(imgs)):
                # load frame-by-frame
                # _, frame = cap.read()
                frame = cv2.imread(os.path.join(img_dir, imgs[current_frame_index]))
                ...
                frame_name = f'{current_frame_index:05d}' + '.png'
...
def get_arguments():
    ...
    parser.add_argument('--img', help='img folder.', default=None)
    parser.add_argument(
        '-m',
        '--mask_dir',
        help=
